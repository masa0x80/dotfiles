...
local image_zoom_script =
	[=[<img src='_' onerror="if(!window._imgClick){window._imgClick=true;document.addEventListener('click',function(e){var t=e.target;if(t.tagName==='IMG'){if(t.classList.contains('enlarged')){t.classList.remove('enlarged');t.style.width=''}else{t.classList.add('enlarged');var cw=(t.closest('.markdown-body')||document.body).clientWidth;var nw=t.naturalWidth;if(nw&&nw>cw){t.style.width=nw+'px'}else{t.style.width='100%'}}e.preventDefault();e.stopPropagation()}})}" style="display:none;">]=]
-- GitHub-stle Alerts
local alert_script =
