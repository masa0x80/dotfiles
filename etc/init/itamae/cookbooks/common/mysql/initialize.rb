...
execute 'initialize mysql' do
  ENV['TEMP_PASSWORD'] = 'Passw0rd!'
  command <<-'EOF'
    mysql -uroot -p"$(sudo grep 'password is generated' /var/log/mysqld.log | cut -d' ' -f 11 | sed -e 's/"/\\"/g')" -e "ALTER USER 'root'@'localhost' IDENTIFIED BY '${TEMP_PASSWORD}';" --connect-expired-password
